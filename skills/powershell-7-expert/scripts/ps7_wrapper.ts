...
export class PowerShell7Manager {
  ...
  private async executeInlineCommand(command: string, options?: PowerShellOptions): Promise<string> {
    return new Promise((resolve, reject) => {
      const args: string[] = [];
  ...
  async checkPowerShell7Installed(): Promise<boolean> {
    try {
      await this.executeInlineCommand('$PSVersionTable.PSVersion');
      return true;
    } catch {
      return false;
    }
  ...
  async getPowerShell7Version(): Promise<string> {
    const result = await this.executeInlineCommand('$PSVersionTable.PSVersion.ToString()');
    return result.trim();
  }

  async checkModuleInstalled(moduleName: string): Promise<boolean> {
    const result = await this.executeInlineCommand(
